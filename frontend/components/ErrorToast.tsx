...
export function ErrorToast({ message, onClose }: ErrorToastProps) {
  ...
  return (
    <div
      className={`flex items-center gap-3 p-4 rounded-lg border-2 shadow-lg transition-all duration-300 ${
        colors[message.type]
  ...
  return (
    <div className="fixed top-4 right-4 z-50 space-y-2 max-w-md">
      {messages.map(message => (
        <ErrorToast key={message.id} message={message} onClose={onClose} />
      ))}
    </div>
  );
}
